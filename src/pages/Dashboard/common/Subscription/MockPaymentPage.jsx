...
const MockPaymentPage = () => {
  ...
  const { mutateAsync: pay, isPending } = useMutation({
    mutationFn: async () => {
      const { data } = await axiosSecure.post("/subscription/purchase", {
        planId: plan._id,
    ...
    onSuccess: (data) => {
      ...
      navigate("/dashboard/subscription/success", { state: { subscription: data } });
    },
    onError: (err) => {
    ...
    <div className="max-w-lg mx-auto">
      <div className="bg-base-100 border border-base-200 rounded-2xl shadow-xl p-6 md:p-8">
        ...
        <p className="text-sm text-base-content/60 mb-6">
          {plan.name} Plan &middot; {plan.duration} Month{plan.duration > 1 ? "s" : ""} &middot;{" "}
